...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	aquabash: {
		...
		pp: 15,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, horn: 1, bigpecksboost: 1 },
		secondary: {
			chance: 20,
			volatileStatus: 'flinch',
		},
	...
	aquafang: {
		...
		pp: 15,
		priority: 0,
		flags: { bite: 1, contact: 1, protect: 1, mirror: 1, metronome: 1, bigpecksboost: 1 },
		secondary: {
			chance: 20,
			volatileStatus: 'flinch',
		},
	...
	berrysmash: {
		...
		pp: 5,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, hammer: 1, bigpecksboost: 1 },
		onTry(pokemon) {
			return !!pokemon.getItem().isBerry;
		},
	...
	brambleblast: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, arrow: 1, bigpecksboost: 1 },
		secondary: {
			chance: 30,
	...
	cheapshot: {
		...
		pp: 20,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, pulse: 1, bigpecksboost: 1 },
		secondary: null,
		target: "normal",
		type: "Dark",
		contestType: "Tough",
	...
	courtchange: {
		...
		onHitField(target, source) {
			const sideConditions = [
				'mist', 'lightscreen', 'reflect', 'spikes', 'safeguard', 'tailwind', 'toxicspikes', 'stealthrock', 'creepingthorns', 'waterpledge', 'firepledge', 'grasspledge', 'stickyweb', 'auroraveil', 'luckychant', 'gmaxsteelsurge', 'gmaxcannonade', 'gmaxvinelash', 'gmaxwildfire', 'gmaxvolcalith',
			];
			let success = false;
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'creepingthorns', 'stickyweb', 'gmaxsteelsurge'];
	...
	diamondarrow: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, arrow: 1, slicing: 1 },
		ignoreDefensive: true,
		ignoreEvasion: true,
		secondary: null,
		target: "normal",
	...
	infernalparade: {
		...
		flags: { protect: 1, mirror: 1, metronome: 1 },
		onModifyMove(move, source, target) {
			if (!target || !move.secondary) return;
			move.secondary.chance = (target.status || target.hasAbility('comatose')) ? 50 : 30;
		},
		secondary: {
			chance: 30,
			status: 'brn',
	...
	mortalspin: {
		...
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				...
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'creepingthorns', 'stickyweb', 'gmaxsteelsurge'];
	...
	plasmapulse: {
		...
		basePowerCallback(pokemon, target, move) {
			if (target.status || target.hasAbility('comatose')) {
				this.debug('Plasma Pulse BP doubled on status');
				return move.basePower * 2;
			}
			return move.basePower;
		},
	...
	ragingfury: {
		...
		pp: 5,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, failinstruct: 1, bigpecksboost: 1 },
		self: {
			volatileStatus: 'lockedmove',
		},
		secondary: null,
	...
	scorchedearth: {
		...
		onModifyType(move, pokemon, target) {
			if (!target) return;
			const fireMod = this.dex.getEffectiveness('Fire', target);
			const groundMod = this.dex.getEffectiveness('Ground', target);
			if (fireMod === groundMod) {
				move.type = 'Fire';
			} else {
				move.type = fireMod > groundMod ? 'Fire' : 'Ground';
			}
		},
		secondary: {
			chance: 10,
	...
	smite: {
		...
		pp: 15,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, nonsky: 1, metronome: 1, bigpecksboost: 1 },
		volatileStatus: 'smackdown',
		condition: {
			noCopy: true,
			onStart(pokemon) {
	...
	whirlingstrikes: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, kick: 1, bigpecksboost: 1 },
		secondary: null,
		target: "normal",
		type: "Flying",
		contestType: "Cool",
	...
	zephyrrush: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, pulse: 1, wind: 1, bigpecksboost: 1 },
		recoil: [33, 100],
		hasCrashDamage: true,
		onMoveFail(target, source, move) {
			this.damage(source.baseMaxhp / 2, source, source, this.dex.conditions.get('High Jump Kick'));
	...
	voltaicshadow: {
		...
		onAfterHit(target, pokemon) {
			...
			const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'creepingthorns', 'stickyweb', 'gmaxsteelsurge', 'sleazyspores', 'luckyroll',
