...
const Abilities = {
  ...
  asoneglastrier: {
    ...
    onSourceAfterFaint(length2, target, source, effect) {
      if (effect && effect.effectType === "Move") {
        this.boost({ atk: length2 }, source, source, this.dex.abilities.get("chillingneigh"));
      }
    },
    isPermanent: true,
  ...
  frozenbunker: {
    ...
    onSetStatus(status, target, source, effect) {
      if (target.species.id !== "arctiglobe" || target.transformed)
        return;
      if (effect?.status) {
  ...
  transfusion: {
    ...
    onSwitchOut(pokemon) {
      for (const target of this.getAllActive()) {
        if (!target.removeVolatile("transfusion"))
          continue;
  ...
  niceface: {
    onStart(pokemon) {
      if (this.field.isTerrain("grassyterrain") && pokemon.species.id === "blobbosnoicce" && !pokemon.transformed) {
  ...
  bereavement: {
    onStart(pokemon) {
      if (pokemon.swordBoost = true) {
        if (pokemon.species.baseSpecies !== "Wyldhaunt" || pokemon.transformed)
          return;
  ...
  taskoriented: {
    ...
    onStart(pokemon) {
      const fastPop = (list, index) => {
        const length2 = list.length;
        if (index < 0 || index >= list.length) {
          throw new Error(`Index ${index} out of bounds for given array`);
        }
        const element = list[index];
        list[index] = list[length2 - 1];
        list.pop();
        return element;
      };
      const sampleNoReplace = (list) => {
        ...
        if (length2 === 0)
          return null;
        const index = this.random(length2);
  ...
  nuclearization: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (nuclearizationUser !== this.activePokemon)
        return;
  ...
  rustedremembrance: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Urnirate" || pokemon.transformed)
        return;
      if (pokemon.hp < pokemon.maxhp) {
    ...
    onResidualOrder: 29,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Urnirate" || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp < pokemon.maxhp) {
        if (pokemon.species.id === "urnirate") {
          pokemon.formeChange("Urnirate-Shattered");
  ...
  goodnight: {
    ...
    onResidual(pokemon) {
      ...
      for (const target2 of pokemon.foes()) {
        if (target2.status === "slp" || target2.hasAbility("comatose") || target2.hasAbility("lethargic") || target2.hasAbility("boardpowerz")) {
          this.damage(target2.baseMaxhp / 8, target2, pokemon);
        }
      }
    },
    rating: 3,
  ...
  surprise: {
    onModifyMovePriority: 1,
    onModifyMove(move, attacker, defender) {
      if (attacker.species.baseSpecies !== "Jakubrik" || attacker.transformed)
        return;
      if (move.category === "Status" && move.id !== "enclose")
        return;
  ...
  mrshadow: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (mrshadowUser !== this.activePokemon)
        return;
  ...
  infernoguardian: {
    ...
    onResidual(pokemon) {
      if (this.field.isWeather("sunnyday") && pokemon.hasAbility("infernoguardian")) {
        this.add("-message", pokemon.name + " brilha sob o sol, tornando-se impenetr\xE1vel!");
        this.add("-activate", pokemon, "move: Safeguard");
        this.field.addPseudoWeather("inferno_guardian_sun");
      }
    },
    // Neutraliza habilidades de oponentes que entrarem
    onSwitchIn(pokemon) {
      if (this.field.getPseudoWeather("inferno_guardian_sun")) {
        this.add("-message", pokemon.name + " n\xE3o pode usar sua habilidade sob este sol ardente!");
  ...
  phantomdoom: {
    ...
    onResidual(pokemon) {
      ...
      if (target && !target.hasAbility("innerfocus")) {
        ...
        this.add("-message", `${target.name} est\xE1 paralisado de medo e n\xE3o pode se mover!`);
      }
    },
    // Se esse Pokémon desmaia por um golpe de contato, o atacante também desmaia
    onDamagingHit(damage, target, source, move) {
      if (target.hp <= 0 && move.flags["contact"]) {
        this.add("-message", `${target.name} amaldi\xE7oou ${source.name} para o submundo!`);
        source.faint();
      }
    },
    name: "Phantom Doom",
    rating: 5,
    num: 125,
    // Defina um número único para a habilidade
    isNonstandard: "Future"
  },
  evasionboost: {
    onModifyAccuracyPriority: -1,
    ...
    onStart(pokemon) {
      pokemon.addVolatile("evasionboost");
    },
    onEnd(pokemon) {
      pokemon.removeVolatile("evasionboost");
    },
    name: "Evas\xE3o Elevada",
    rating: 2,
    isNonstandard: "Future"
  },
  mindlock: {
    onStart(pokemon) {
      ...
      if (foe.lastMove) {
        ...
        foe.volatiles["encore"].duration = 0;
        foe.volatiles["encore"].move = foe.lastMove.id;
        this.add("-message", `${foe.name} foi for\xE7ado a repetir ${this.dex.moves.get(foe.lastMove.id).name}!`);
      }
    },
    ...
    name: "Mind Lock",
    shortDesc: "Trava o oponente no \xFAltimo golpe usado e impede fuga se estiver no ch\xE3o.",
    rating: 5,
    num: 1022,
    isNonstandard: "Future"
  },
  frostbiteveil: {
    ...
    name: "Frostbite Veil",
    rating: 4,
    // Ajuste a avaliação conforme necessário
    num: 1005,
    // Defina um número único para a habilidade
    isNonstandard: "Future"
    // Defina como "Future" ou outro valor apropriado
  },
  bloodshackles: {
    ...
    onStart(pokemon) {
      ...
      for (const target of pokemon.side.foe.active) {
        if (target && target.isActive) {
          this.add("-message", `${target.name} come\xE7a a sangrar devido \xE0 presen\xE7a de ${pokemon.name}!`);
    ...
    onFoeTrapPokemon(pokemon) {
      if (!pokemon.hasAbility("bloodshackles") && pokemon.isAdjacent(this.effectState.target)) {
        pokemon.tryTrap(true);
        this.add("-message", `${pokemon.name} est\xE1 preso pelo sangue amaldi\xE7oado de ${this.effectState.target.name}!`);
      }
    },
    onFoeMaybeTrapPokemon(pokemon, source) {
      if (!source)
  ...
  reincarnation: {
      ...
      if (!pokemon.m.reincarnationCurseApplied) {
        for (const target of pokemon.side.foe.active) {
          if (target && target.isActive && !target.hasType("Ghost")) {
            this.add("-message", `${pokemon.name} lan\xE7ou uma maldi\xE7\xE3o sobre ${target.name}!`);
            this.directDamage(target.maxhp / 4, target, pokemon);
            target.addVolatile("curse");
          }
        }
        pokemon.m.reincarnationCurseApplied = true;
      }
    },
    ...
    onFoeTrapPokemon(pokemon) {
      if (!pokemon.hasAbility("phantomdoom") && pokemon.isAdjacent(this.effectState.target)) {
        pokemon.tryTrap(true);
        this.add("-message", `${pokemon.name} est\xE1 preso por uma aura fantasmag\xF3rica!`);
      }
    },
    ...
    onSourceFaint(target, source, effect) {
      ...
      const reincarnatedPokemon = new import_sim.Pokemon({
        ...targetSet,
    ...
    name: "Reincarnation",
    shortDesc: "Aplica Curse no final do 1\xBA turno, impede trocas e reencarna inimigos derrotados.",
    rating: 5,
    num: 1007,
    isNonstandard: "Future"
  },
  ...
  cursedgold: {
    onDamagingHit(damage, target, source, move) {
      if (target.item === "stickybarb" && move.flags["contact"]) {
        this.add("-message", `${source.name} pegou a Maldi\xE7\xE3o Dourada!`);
  ...
  statusthief: {
      ...
      for (const target of pokemon.side.foe.active) {
        if (target && target.isActive) {
          ...
          for (const stat in target.boosts) {
            if (target.boosts[stat] > 0) {
              boosts[stat] = target.boosts[stat];
    ...
    onFoeSwitchIn(target) {
      if (target.item) {
        ...
        this.add("-message", `${target.name} perdeu seu ${target.item} devido \xE0 Status Thief!`);
        target.clearItem();
      }
    },
    name: "Status Thief",
    rating: 4.5,
    num: 1006,
    // Substitua pelo número que você quiser atribuir à habilidade
    isNonstandard: "Future"
  },
  fairyenchanter: {
    ...
    onStart(pokemon) {
      ...
      if (target && target.isActive) {
        ...
        this.add("-message", `${target.name} perdeu 1 est\xE1gio de ataque e ataque especial!`);
        this.boost({ atk: 1, spa: 1 }, pokemon);
        this.add("-message", `${pokemon.name} ganhou 1 est\xE1gio de ataque e ataque especial!`);
      }
    },
  ...
  mindcorruption: {
    ...
    onStart(pokemon) {
      for (const target of pokemon.side.foe.active) {
        if (target && target.isActive) {
          this.add("-message", `${pokemon.name} come\xE7a a corromper a mente de ${target.name}!`);
          if (!target.mindCorruptionTurns) {
            target.mindCorruptionTurns = 0;
          }
        }
      }
    },
    ...
    onResidual(pokemon) {
      for (const target of pokemon.side.foe.active) {
        if (target && target.isActive) {
          if (typeof target.mindCorruptionTurns !== "number") {
            target.mindCorruptionTurns = 0;
          }
          target.mindCorruptionTurns++;
          this.add("-message", `${target.name} est\xE1 sob influ\xEAncia da corrup\xE7\xE3o mental (${target.mindCorruptionTurns}/5 turnos).`);
          if (target.mindCorruptionTurns >= 4) {
            ...
            const clonedPokemon = new import_sim.Pokemon({
              ...target.set,
              name: target.name
            }, allySide);
            ...
            this.add("-message", `${target.name} agora pertence \xE0 equipe de ${pokemon.side.name}!`);
            target.faint();
          }
        }
      }
  ...
  hellishvoid: {
    ...
    rating: 4.5,
    num: 1025,
    // Número pode ser ajustado conforme necessário
    isNonstandard: "Future",
  ...
  toxicreign: {
    ...
    onFaint(target, source, effect) {
      if (source && source !== target && target.status === "psn" || target.status === "tox") {
        ...
        this.add("-message", `${source.name} absorveu a ess\xEAncia do veneno e se curou completamente!`);
      }
    }
  },
  adaptivearmor: {
    shortDesc: "Reduz dano de ataques neutros e resistidos. Moves Normais viram STAB e n\xE3o erram.",
    name: "Adaptive Armor",
    rating: 4.5,
    num: 1019,
    // Número pode ser ajustado conforme necessário
  ...
  permafrost: {
    shortDesc: "Invoca Hail ao entrar. Em Hail, +1 Def/SpD oculto, 30% de congelar oponente e -30% precis\xE3o dele.",
    name: "Permafrost",
    rating: 4.5,
    num: 1020,
    // Número pode ser ajustado conforme necessário
  ...
  electricascension: {
    shortDesc: "Absorve golpes el\xE9tricos, converte Normal em El\xE9trico (+50% poder), buffa evas\xE3o e usa Thunder triplo se HP < 1/4.",
    ...
    onTryHit(target, source, move) {
      if (move.type === "Electric") {
        ...
        this.add("-message", `${target.name} absorveu a energia el\xE9trica!`);
    ...
    onResidual(pokemon) {
      if (pokemon.hp <= pokemon.maxhp / 4) {
        this.add("-message", `${pokemon.name} atingiu sua Ascens\xE3o El\xE9trica!`);
        this.boost({ evasion: 6 }, pokemon);
        this.actions.useMove(Dex.moves.get("thunder"), pokemon);
        const thunderMove = this.dex.getActiveMove("thunder");
        thunderMove.basePower *= 3;
      }
    }
  },
  lovetrap: {
    ...
    onDamagingHit(damage, target, source, move) {
      if (source.hp > 0) {
        this.add("-message", `${source.name} n\xE3o pode fugir e est\xE1 ficando mais fraco!`);
        this.boost({ atk: -1, spa: -1 }, source, target, this.dex.abilities.get("lovetrap"));
      }
    },
    shortDesc: "Fim do turno: advers\xE1rio fica Infatuated e Confused. Se atacar, \xE9 for\xE7ado a trocar. \xDAltimo mon perde 1 est\xE1gio de Atk e SpA.",
    name: "Love Trap",
    rating: 5,
    num: 1022,
    // Número pode ser ajustado conforme necessário
  ...
  supremacy: {
    ...
    onModifySecondaries(secondaries, target, source, move) {
      if (target !== source) {
        this.debug("Supremacy bloqueia efeitos secund\xE1rios");
    ...
    onModifyAccuracy(accuracy, target, source, move) {
      if (move.priority > 0) {
        this.debug("Supremacy reduz precis\xE3o de moves de prioridade");
        return 30;
      }
    },
    ...
    onTrapPokemon(pokemon) {
      ...
      this.add("-message", `${pokemon.name} \xE9 imune a efeitos de aprisionamento devido \xE0 Supremacy!`);
    },
    ...
    onResidual(pokemon) {
      if (pokemon.abilityState.bulkUpUsed)
        return;
    ...
    shortDesc: "Todos os golpes s\xE3o cr\xEDticos. Imune a efeitos secund\xE1rios, golpes de status, trapping, weather, hazards, e moves priorit\xE1rios t\xEAm precis\xE3o 30% contra ele. Invoca Endure ao entrar e Bulk Up no final do 1\xBA turno.",
    name: "Supremacy",
    rating: 5,
    num: 1024,
    // Número pode ser ajustado conforme necessário
  ...
  abilitylock: {
    ...
    onStart(pokemon) {
      const target = pokemon.side.foe.active[0];
      if (!target || target.ability === "truant")
        return;
    ...
    shortDesc: "Ao entrar, muda a habilidade do oponente para Normalize e, no turno seguinte, para Truant at\xE9 que ele saia.",
    name: "Ability Lock",
    rating: 5,
    num: 1040,
    // Número ajustável
  ...
  rainfirestorm: {
    ...
    rating: 4.5,
    num: 1021,
    // Número fictício, ajuste conforme necessário
    isNonstandard: "Future"
  },
  divinecore: {
    ...
    shortDesc: "Muda o tipo com a Plate equipada. Moves Normal viram do tipo correspondente. O item n\xE3o pode ser removido.",
    name: "Divine Core",
    rating: 5,
    num: 1028,
    // Número pode ser ajustado conforme necessário
  ...
  doomprophecy: {
    ...
    shortDesc: "No fim do turno: 10% de desmaiar o oponente; se falhar, 8% de desmaiar o usu\xE1rio.",
    name: "Doom Prophecy",
    rating: 5,
    num: 1026,
    // Número pode ser ajustado conforme necessário
    ...
    onResidual(pokemon) {
      for (const target of pokemon.side.foe.active) {
        if (!target || target.fainted)
          continue;
        if (this.randomChance(11, 100)) {
          this.add("-message", `${target.name} foi amaldi\xE7oado pela Profecia Sombria!`);
          target.faint();
          return;
        }
      }
      if (this.randomChance(7, 100)) {
  ...
  nopivotblock: {
    ...
    condition: {
      duration: 3,
      // Dura 3 turnos, impedindo movimentos de pivô durante todo esse tempo
      onStart(target) {
    ...
    shortDesc: "Impede trocas e movimentos de piv\xF4 por 3 turnos.",
    name: "No Pivot Block",
    rating: 4.5,
    num: 1020,
  ...
  lockdown: {
    ...
    shortDesc: "Ap\xF3s usar um Stalling Move, impede todos os Pok\xE9mon de agirem por 3 turnos. S\xF3 ativa uma vez por batalha.",
    name: "Lockdown",
    rating: 5,
    num: 1021,
    isNonstandard: "Future"
  ...
  primordialguard: {
    ...
    onDamage(damage, target, source, effect) {
      if (target.volatiles["primordialguard"]) {
        ...
        target.removeVolatile("primordialguard");
        if (reflectedDamage > 0 && source) {
          this.damage(reflectedDamage, source, target, effect);
        }
    ...
    shortDesc: "Primeiro dano recebido ao entrar \xE9 10% do HP m\xE1ximo; 90% \xE9 refletido.",
  ...
  shadowcurse: {
    shortDesc: "Invoca midnight ao entrar. Em Hail, +1 Def/SpD oculto, 30% de congelar oponente e -30% precis\xE3o dele.",
    name: "Shadow Curse",
    rating: 4.5,
    num: 1020,
    // Número pode ser ajustado conforme necessário
    ...
    onResidual(target) {
      ...
      if (this.field.isWeather("midnight") && foe && this.randomChance(25, 100)) {
        this.add("-message", `${foe.name} foi amaldi\xE7oado pelas sombras!`);
        ...
        this.add("-message", `${foe.name} sofreu dano da Maldi\xE7\xE3o imediatamente!`);
      }
    }
  },
  undyingvolt: {
    onDamage(damage, target, source, effect) {
      if (target.volatiles["undyingvolt"])
        return;
      if (damage >= target.hp) {
  ...
  anarchyaura: {
    ...
    condition: {
      onBeforeMove(pokemon, target, move) {
        if (pokemon.moveSlots.length > 1) {
          const possibleMoves = pokemon.moveSlots.map((slot) => slot.id).filter((id) => id !== move.id);
    ...
    shortDesc: "Todos os Pok\xE9mon no campo ignoram comandos e escolhem um move aleat\xF3rio, exceto o selecionado pelo treinador.",
    name: "Anarchy Aura",
    rating: 5,
    num: 1027,
    isNonstandard: "Future"
