...
const Items = {
  ...
  efficientpick: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (degradationUser !== this.activePokemon)
        return;
      if (!this.activePokemon || this.activePokemon.species.id !== "Primiteve")
        return;
  ...
  vortexorb: {
    ...
    onModifySpAPriority: 5,
    onModifySpA(spa, pokemon) {
      if (pokemon.species.id !== "Paralotl")
        return;
      if (pokemon.hp <= pokemon.maxhp / 2) {
        return this.chainModify(2);
      }
  ...
  cursedcharm: {
    ...
    onResidualSubOrder: 3,
    onResidual(pokemon) {
      if (pokemon.species.id !== "Begleamer" || pokemon.species.id !== "Sneevil")
        return;
      pokemon.addVolatile("torment");
    },
    fling: {
    ...
    onModifyDamage(damage, source, target, move) {
      if (source.species.name === "Begleamer" || source.species.name === "Sneevil")
        return;
      return this.chainModify(1.5);
    },
    num: 67339,
  ...
  berserkroot: {
    ...
    onAfterMoveSecondarySelfPriority: -1,
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (pokemon.species.id !== "rooterot")
        return;
      if (move.totalDamage) {
  ...
  iceskates: {
    ...
    onModifySpe(spe, pokemon) {
      if (pokemon.species.name === "Novickate" || pokemon.species.name === "Verdicent")
        return;
      return this.chainModify([5324, 4096]);
    },
    onAfterMoveSecondarySelf(source, target, move) {
