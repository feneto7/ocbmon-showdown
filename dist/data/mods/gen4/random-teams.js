...
class RandomGen4Teams extends import_random_teams.default {
  constructor(format, prng) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Ice: (movePool, moves, abilities, types, counter) => !counter.get("Ice") && (!types.has("Water") || !counter.get("Water")) || movePool.includes("blizzard"),
  ...
  shouldCullMove(move, types, moves, abilities, counter, movePool, teamDetails, species, isLead) {
    ...
    switch (move.id) {
      ...
      case "focuspunch":
        return { cull: !moves.has("substitute") || counter.damagingMoves.size < 2 || moves.has("hammerarm") || moves.has("focusblast") };
      case "lightscreen":
