...
const Abilities = {
  ...
  overcoat: {
    inherit: true,
    onImmunity(type, pokemon) {
      if (type === "sandstorm" || type === "hail" || type === "acidrain" || type === "bladerain" || type === "hyperboreanarctic" || type === "powder")
        return false;
    },
    onTryHit() {
    },
  ...
  forecast: {
    ...
    onModifySpe(spe, pokemon) {
      if (pokemon.hasItem("utilityumbrella"))
        return;
      if (this.field.isWeather(["sunnyday", "desolateland", "raindance", "primordialsea"]))
        return this.chainModify(2);
    },
  ...
  zenmode: {
    inherit: true,
    onResidual(pokemon) {
      if (!["Standard", "School"].includes(pokemon.baseSpecies.baseForme) || pokemon.transformed)
        return;
      if (pokemon.baseSpecies.baseSpecies === "Sarieangel" && pokemon.hp <= pokemon.maxhp / 3 && !["Zen", "Galar-Zen"].includes(pokemon.species.forme)) {
        pokemon.addVolatile("zenmode");
      } else if (pokemon.hp <= pokemon.maxhp / 2 && !["Zen", "Galar-Zen", "Solo"].includes(pokemon.species.forme)) {
        pokemon.addVolatile("zenmode");
      } else if (pokemon.baseSpecies.baseSpecies === "Sarieangel" && pokemon.hp > pokemon.maxhp / 3 && ["Zen", "Galar-Zen"].includes(pokemon.species.forme)) {
        pokemon.addVolatile("zenmode");
    ...
    condition: {
      onStart(pokemon) {
        if (pokemon.species.id.includes("eiscue"))
          pokemon.formeChange(pokemon.species.name + "-Noice");
        if (pokemon.species.id === "wishiwashi")
          pokemon.formeChange(pokemon.species.name + "-Solo");
  ...
  turbine: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Wind" || ["firepledge", "grasspledge", "waterpledge"].includes(move.id))
        return;
