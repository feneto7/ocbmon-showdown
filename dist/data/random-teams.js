...
class RandomTeams {
  constructor(format, prng) {
    ...
    this.randomMemeSets = require("./meme-sets.json");
  ...
  cullMovePool(types, moves, abilities, counter, movePool, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (species.id === "jirachi")
      this.incompatibleMoves(moves, movePool, "bodyslam", "healingwish");
    if (species.baseSpecies !== "Basculin")
      this.incompatibleMoves(moves, movePool, "aquajet", "flipturn");
  ...
  randomMemeTeam() {
    ...
    const sets = this.multipleSamplesNoReplace(indexes, 6).map((index) => this.randomMemeSets[index]);
