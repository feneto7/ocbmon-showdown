...
Config.potd = '${species.id}'`);
  ...
  addhtmlbox(target2, room2, user2, connection2, cmd) {
    ...
    target2 = Chat.collapseLineBreaksHTML(target2);
    if (!BOT_GROUPS.includes(user2.tempGroup)) {
...
${buf}`);
  ...
  async hotpatch(target2, room2, user2, connection2, cmd) {
    ...
    const [hotpatchTarget, options] = target2.split(",").map(toID);
    target2 = hotpatchTarget;
    try {
...
${e.stack}`);
    ...
    if (this.room && (options || []).includes("notify")) {
      this.room.add(`|raw|<p>${user2.name} hotpatched ${target2}</p>`);
    }
  },
  hotpatchhelp: [
