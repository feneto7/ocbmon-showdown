...
`;
          if (showDetails) {
            ...
            if (move.flags["dance"] && dex.gen >= 7)
              details["&#10003; Dance move"] = "";
            if (move.flags["slicing"] && dex.gen >= 8)
              details["&#10003; Slicing move"] = "";
            if (move.flags["wind"] && dex.gen >= 9)
              details["&#10003; Wind move"] = "";
            if (move.flags["kick"] && dex.gen >= 8)
              details["&#10003; Kick move"] = "";
            if (move.flags["bone"] && dex.gen >= 8)
              details["&#10003; Bone move"] = "";
            if (move.flags["hammer"] && dex.gen >= 8)
              details["&#10003; Hammer move"] = "";
            if (dex.gen >= 7) {
...
`;
  ...
  coverage(target, room, user) {
    ...
    if (!dispTable) {
      ...
      const icon = {};
      for (const type of dex.types.names()) {
        icon[type] = `<img src="//${Config.routes.client}/sprites/types/${type}.png" width="32" height="14">`;
  ...
  innate(target) {
    if (!this.runBroadcast())
      return;
    if (!target)
      return this.parse("/help innate");
    target = target.toLowerCase().replace(/\s/g, "");
    switch (target) {
      case "list":
        return this.sendReplyBox(`List of Wack Pok&eacute;mons with a coded innate ability:<br /> Achillesder, Aegislash, Alphanne, Angry Man Jew, Audinette, Audino, Azathoth, Bersecules, Borosu, Castform, Cclefable, Cclefairy, Crabblante, DQ Dragon Lord, Dragurve, Eiscue, Empty Anne, F-00, Feiscue, God, Gowen, HIM Chaos, HIM Cyber, HIM Divine, HIM F, HIM G, HIMless, HIM Magma, HIM Nuclear, HIM Paper, HIM Void, HIM W, HIM Wall, HIM Wind, Indeedee, Jekhyde, Kawainnocent, Lunar Guardian, Maidcoresh, ManVaccine, Mecha Zaydolf, Mestwi, Meta Knight, Mlavagun, Msteamboatle, Neapolitaneiscue, Pincurchin, Rillaboom, Sandaconda, SCPee 096, Skeleton, Tank Top Mastah, Tapu Bulu, Tapu Fini, Tapu Koko, Tapu Lele, Tardida, Tikcofagrigus, Torterra, True HIM, Watch Doggo Man, Wishiwashi`);
      case "tapukoko":
  ...
  innatehelp: [
    `/innate list - Display all Pok\xE9mons with an innate ability.`,
    `/innate [pokemon] - Display the innate ability of a Pok\xE9mon, if it has one.`,
