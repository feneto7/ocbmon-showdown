...
const commands = {
  ...
  emojilevel(target, room, user) {
    ...
    if (room.settings.emojiLevel && room.settings.emojiLevel.length <= 1 && !room.auth.atLeast(user, room.settings.emojiLevel) && // Upper Staff should probably be able to set /modchat & in secret rooms
    !user.can("bypassall")) {
    ...
    switch (target) {
      ...
      default:
        ...
        if (emojiLevelHigherThanUserRank || !Users.Auth.hasPermission(user, "emojilevel", target, room)) {
    ...
    if (!room.settings.emojiLevel) {
      ...
      const emojiLevelSetting = import_lib.Utils.escapeHTML(room.settings.emojiLevel);
