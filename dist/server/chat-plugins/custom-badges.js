...
var custom_badges_exports = {};
__export(custom_badges_exports, {
  commands: () => commands
});
module.exports = __toCommonJS(custom_badges_exports);
var import_badges2 = require("./badges");
const CHUNK_SIZE = 4;
const createTeam = async (teamToBadge) => {
  const team = {};
  await Promise.all(Object.entries(teamToBadge).map(async ([sideName, badgeID]) => {
    const badge = await import_badges2.Badges.getBadge(badgeID);
    if (badge) {
      team[sideName] = badge;
    }
  }));
...
const createTeamHtml = (teamName, team) => {
  ...
  chunkedBadges.forEach((badgeChunk) => {
    teamHtml += "<tr>";
    teamHtml += badgeChunk.map(([sideName, badge]) => "<td>" + import_badges2.Badges.createBadgeHtml({ ...badge, badge_id: sideName }, false) + "</td>").join("");
    teamHtml += "</tr>";
  });
  teamHtml += "</table></details>";
  return teamHtml;
};
const joinTeam = async (user, teamName, teamSide) => {
  ...
  if (userBadges) {
    ...
    await Promise.all(
      existingTeamBadges.map(
        (existingTeamBadge) => import_badges2.Badges.removeBadgeFromUser(user.id, existingTeamBadge.badge_id, user, true)
      )
    );
  }
  await import_badges2.Badges.addBadgeToUser(user.id, badge.badge_id, user, true);
  return badge;
};
const commands = {
  sidejoin: "badgeteam",
  ...
  badgeteam: {
    async list() {
      ...
      this.runBroadcast();
      return this.sendReplyBox("<b><u>Team Badges</b><br />" + Object.entries(teams).map(([teamName, team]) => createTeamHtml(teamName, team)).join("<br />"));
    },
    async join(target, room, user) {
      ...
      return this.sendReplyBox(`Successfully joined team ${teamSide}: ${import_badges2.Badges.createBadgeHtml(badge, false)}`);
    },
    "": "help",
    help() {
...
//# sourceMappingURL=custom-badges.js.map
