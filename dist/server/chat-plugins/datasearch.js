...
function runDexsearch(target, cmd2, canAll, message, isTest) {
  ...
  const allSpeciesTags = ["blobbokind", "krackokind", "weedlekind", "inferior", "mythical", "restrictedlegendary", "sublegendary"];
  let showAll = false;
  let sort = null;
  let megaSearch = null;
  let gmaxSearch = null;
  ...
  for (const species of mod.species.all()) {
    ...
    if (species.gen <= mod.gen && (nationalSearch && species.isNonstandard && !["Custom", "Glitch", "Pokestar", "Future"].includes(species.isNonstandard) || (species.tier !== "Unreleased" || unreleasedSearch) && species.tier !== "Illegal") && (nationalSearch || species.isNonstandard !== "Past") && (!species.tier.startsWith("CAP") || capSearch) && megaSearchResult && gmaxSearchResult && fullyEvolvedSearchResult) {
  ...
  for (const alts of searches) {
    ...
    for (const mon in dex) {
      ...
      for (const tagSearch in alts.tags) {
        const tag = import_tags.Tags[toID(tagSearch)];
