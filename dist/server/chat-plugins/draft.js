...
const drafts = JSON.parse(
  (0, import_lib.FS)(DRAFT_PATH).readIfExistsSync() || "{}"
);
const saveDrafts = () => {
  (0, import_lib.FS)(DRAFT_PATH).writeUpdate(() => JSON.stringify(drafts));
};
...
const Drafts = new class {
  canCreateDraft(context) {
  ...
  createDraft(context, user, nameInput, formatInput, budgetInput, inviteOnlyInput) {
  ...
  getDraft(draftName, throwError = false) {
  ...
  canManageDraft(draft, user) {
    return draft.adminId === user.id;
  }
  addPlayer(user, draftName, userName, position = -1) {
...
const Badges = new class {
  ...
  createBadgeHeaderButtons(currentPage) {
    ...
    const refresh2 = `<button class="button" name="send" value="/j view-badge-${currentPage}" style="float: right"> <i class="fa fa-refresh"></i> Refresh</button>`;
    return toLink(`<div style="line-height:25px">${buf.join(" / ")}${refresh2}</div><hr />`);
  }
...
const pages = {
  draft(query, user, connection) {
    return pages.drafts.view.call(this, query, user, connection);
  },
  drafts: {
