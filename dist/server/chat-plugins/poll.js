...
class Poll extends Rooms.MinorActivity {
  ...
  generateVotes(user) {
    ...
    output += ` <strong style="font-size:11pt">${Poll.getQuestionMarkup(this.question, this.supportHTML, this.ratingRequirement)}</strong></p>`;
    if (this.multiPoll) {
  ...
  static generateResults(options, room, ended = false, choice = null) {
    ...
    let output = `<div class="infobox"><p style="margin: 2px 0 5px 0">${icon} <strong style="font-size:11pt">${this.getQuestionMarkup(options.question, options.supportHTML, options.ratingRequirement)}</strong></p>`;
  ...
  static getQuestionMarkup(question, supportHTML = false, ratingRequiredment) {
    if (supportHTML)
      return question;
    let ratingText = "";
    if (ratingRequiredment) {
      const format = Dex.formats.get(ratingRequiredment.format);
      ratingText = ` (>${ratingRequiredment.minimumRating} in ${format.name})`;
    }
    return Chat.formatText(`${question}${ratingText}`);
  }
  ...
  poll: {
    ...
    new(target, room, user, connection, cmd, message) {
      ...
      if (params.length > 1 && requiresRating) {
        ...
        const actualFormat = Dex.formats.get(format);
        if (!actualFormat.exists) {
          return this.errorReply(this.tr`Non-existent format "${format}" provided.`);
        }
        if (Number.isNaN(rating) || rating < 1e3) {
          return this.errorReply(this.tr`Invalid rating requirement "${params[1]}" provided.`);
        }
