...
const commands = {
  teams: {
    ...
    async save(target, room, user, connection, cmd) {
      ...
      const teamID = isEdit ? targets.shift() : void 0;
...
const pages = {
  ...
  teams: {
    ...
    upload(query, user, connection) {
      ...
      buf += `<formatselect name="format" format="gen8cloveronlyou">[Gen 8 Clover Only] OU</formatselect><br />`;
    ...
    async searchpublic(query, user, connection) {
      ...
      if (!query.map(toID).filter(Boolean).length || isPersonal && query.length === 1) {
        ...
        buf += `Team format: <formatselect name="tier" format="gen8cloveronlyou">[Gen 8 Clover Only] OU</formatselect><br /><br />`;
