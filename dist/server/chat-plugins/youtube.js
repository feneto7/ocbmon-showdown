...
class GroupWatch extends Rooms.SimpleRoomGame {
  ...
  hints() {
    ...
    if (this.started) {
      ...
      hints.push(`Video is currently at ${Chat.toDurationString(diff)} (${diff / 1e3} seconds)`);
    }
    return hints;
  }
  getStatsDisplay() {
...
class TwitchStream extends Rooms.SimpleRoomGame {
  ...
  static async createStreamWatch(room, channel) {
    ...
    const watchRoom = Rooms.createGameRoom(
      ...
      import_lib.Utils.html`[Twitch Watch] ${data.display_name}`,
      { isPrivate: "hidden" }
    );
    room.saveSettings();
...
const commands = {
  async randchannel(target, room, user) {
    room = this.requireRoom(room?.roomid);
  ...
  youtube: {
    ...
    async groupwatch(target, room, user) {
      ...
      room.add(
        `|uhtml|${gameRoom.roomid}|<button class="button" name="send" value="/j ${gameRoom.roomid}">Join the ongoing group watch!</button>`
      );
  ...
  twitch: {
    ...
    async watch(target, room, user) {
      ...
      user.joinRoom(gameRoom);
      room.add(
        `|uhtml|ts-${gameRoom.roomid}|<button class="button" name="send" value="/j ${gameRoom.roomid}">Join the ongoing stream watch!</button>`
      ).update();
    },
    start(target, room) {
      room = this.requireRoom();
