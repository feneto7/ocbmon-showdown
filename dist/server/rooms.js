...
class BasicRoom {
  ...
  getUserCount() {
    return Object.values(this.users).filter((user) => user.named && user.connected).length;
  }
  getUserList() {
    let buffer = "";
    let counter = 0;
...
class GlobalRoomState {
  ...
  getTotalUserCount() {
    return Array.from(Users.users.values()).filter((user) => user.named && user.connected).length;
  }
  getRooms(user) {
    const roomsData = {
      chat: [],
...
`).join("");
  ...
  autojoinRooms(user, connection) {
    ...
    if (!includesLobby && Config.serverid !== "clodown")
      user.send(`>lobby
|deinit`);
  }
