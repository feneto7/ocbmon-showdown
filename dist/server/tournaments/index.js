...
const officialTournamentResults = JSON.parse(
  (0, import_lib.FS)("config/chat-plugins/official-tournaments.json").readIfExistsSync() || "{}"
);
const saveOfficialTournamentResults = () => {
  (0, import_lib.FS)("config/chat-plugins/official-tournaments.json").writeUpdate(() => JSON.stringify(officialTournamentResults));
};
...
${reason}` : `.`}`);
  ...
  async acceptChallenge(user, output) {
    ...
    if (!player.pendingChallenge)
      return;
    if (this.teamLock) {
      if (!player.lockedTeam)
        player.lockedTeam = ready.settings.team;
      user.sendTo(this.room, '|tournament|update|{"teamLocked":true}');
      const opponent = this.playerTable[from.id];
      if (!opponent.lockedTeam)
        opponent.lockedTeam = challenge.team;
  ...
  onOfficialTournamentEnd(winners) {
...
- ${reasons.replace(/\n/g, "\n- ")}`);
    ...
    teamlock(target, room, user) {
      ...
      if (this.meansYes(target)) {
        if (tournament.teamLock)
          throw new Chat.ErrorMessage("This tournament is already team-locked.");
        ...
        if (!tournament.teamLock)
          throw new Chat.ErrorMessage("Team lock is already off for this tournament.");
    ...
    results(target) {
      ...
      if (targetId.length) {
        ...
        const lines = Object.entries(officialTournamentResults).sort((resultA, resultB) => resultB[1].tournaments.length - resultA[1].tournaments.length).map(([
          playerId,
          tournamentResults
