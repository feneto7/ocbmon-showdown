...
class Species extends import_dex_data.BasicEffect {
  constructor(data) {
    ...
    if (!this.gen && this.num >= 1) {
      if (this.num >= 999e3 && this.num <= 999999) {
        this.gen = 8;
...
class DexSpecies {
  ...
  getByID(id) {
    ...
    if (id && this.dex.data.Pokedex.hasOwnProperty(id)) {
      ...
      const speciesStatuses = this.dex.data.Conditions[(0, import_dex_data.toID)(species.name)];
      if ((0, import_dex_data.toID)(species.name) !== (0, import_dex_data.toID)(species.baseSpecies) && speciesStatuses !== void 0) {
        let key;
        for (key in speciesStatuses) {
          if (!(key in species))
            species[key] = speciesStatuses[key];
        }
      }
