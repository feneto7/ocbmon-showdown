	...
	stone(target) {
		...
		for (const aStone of toDisplay) {
			...
			if (deltas.type && deltas.type !== 'mono') {
				buf += `<span class="col typecol"><img src="//${Config.routes.client}/sprites/types/${deltas.type}.png" alt="${deltas.type}" height="14" width="32"></span> `;
			} else {
	...
	franticfusions(target, room, user, connection, cmd) {
		...
		if (species.types && fusion.name.length) {
			for (const type of species.types) {
				buf += `<img src="//${Config.routes.client}/sprites/types/${type}.png" alt="${type}" height="14" width="32">`;
			}
		}
		buf += '</span> ';
		if (dex.gen >= 3) {
